...
import { Search, FileText, TrendingUp, Clock, ArrowRight, AlertCircle } from 'lucide-react'
...
const STATUS_COLORS: Record<string, string> = {
  ...
  submitted: 'text-gold-400 bg-gold-500/10 border-gold-500/20',
  under_review: 'text-teal-400 bg-teal-500/10 border-teal-500/20',
  awarded: 'text-green-400 bg-green-500/10 border-green-500/20',
  ...
  return (
    <Layout>
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {profileIncomplete && (
          <div className="mb-6 px-5 py-4 rounded-xl bg-gold-500/8 border border-gold-500/20 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <AlertCircle size={18} className="text-gold-400 shrink-0" />
              <div>
                <p className="font-sans font-medium text-sm text-gold-300">Complete your profile to unlock AI matching</p>
                <p className="font-sans text-xs text-gold-400/70 mt-0.5">Add your sectors, description, and location to get personalized grant recommendations.</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            { value: recentGrants.length > 0 ? '10K+' : '0', label: 'Available Grants', icon: Search, color: 'gold' },
            ...
            <div key={label} className="card p-5">
              <div className="flex items-start justify-between mb-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center
                  ${color === 'gold' ? 'bg-gold-500/10' : 'bg-teal-500/10'}`}>
                  <Icon size={15} className={color === 'gold' ? 'text-gold-400' : 'text-teal-400'} />
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {loading ? (
              <div className="space-y-3">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="card p-5 h-36 animate-pulse bg-navy-700/50" />
                ))}
              </div>
            ) : recentGrants.length === 0 ? (
              <div className="card p-8 text-center">
                <Search size={24} className="text-slate-600 mx-auto mb-3" />
                <p className="font-sans text-sm text-slate-500">No grants in database yet.</p>
                <p className="font-sans text-xs text-slate-600 mt-1">Use "Find Grants" to sync from Grants.gov.</p>
          ...
          <div className="space-y-6">
                  ...
                  {upcomingDeadlines.map((sg) => {
                    ...
                    return (
                      <div key={sg.id} className="card p-4 flex items-start gap-3">
                        <div className={`mt-0.5 w-2 h-2 rounded-full shrink-0 ${urgent ? 'bg-gold-400' : 'bg-teal-400'}`} />
                        <div className="flex-1 min-w-0">
                          <p className="font-sans text-xs font-medium text-slate-300 line-clamp-2">
                            {sg.grant!.title}
                          </p>
                          <p className={`font-mono text-xs mt-1 ${urgent ? 'text-gold-400' : 'text-slate-500'}`}>
                            {dl}
                          </p>
                        </div>
                      </div>
                ...
                <div className="space-y-2">
                  {applications.slice(0, 4).map((app) => (
                    <div key={app.id} className="card p-4">
                      <p className="font-sans text-xs font-medium text-slate-300 line-clamp-1 mb-2">
            ...
            {/* Pipeline summary */}
            {totalPipeline > 0 && (
              <div className="card p-5 bg-gradient-to-b from-navy-700/80 to-navy-800/80">
                <p className="font-sans text-xs text-slate-500 mb-1">Total Pipeline</p>
