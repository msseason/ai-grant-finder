...
import { ArrowRight, Zap, BarChart3, Target, Calendar, Shield, TrendingUp, CheckCircle } from 'lucide-react'
...
const stats = [
  ...
  { value: '50%',     label: 'Avg Success Rate' },
...
const trustPoints = [
  ...
  'Any organization type or sector',
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <section className="relative pt-28 pb-24 px-4 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 left-1/3 w-[600px] h-[400px] rounded-full bg-gold-400/8 blur-3xl" />
          <div className="absolute bottom-0 right-1/4 w-96 h-96 rounded-full bg-teal-500/6 blur-3xl" />
        </div>

        <div className="relative max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-gold-300 bg-gold-50 mb-8 animate-fade-in">
            <span className="w-1.5 h-1.5 rounded-full bg-gold-500 animate-pulse" />
            <span className="font-mono text-xs text-gold-700 tracking-wider uppercase">AI Grant Intelligence Platform</span>
          </div>

          <h1 className="font-display text-5xl sm:text-6xl lg:text-7xl font-semibold text-slate-900 leading-[1.05] tracking-tighter mb-6 animate-fade-up">
            Find & Win Grants
            <br />
            <span className="text-gold-600 italic">With Intelligence</span>
          </h1>

          <p className="font-sans text-lg text-slate-600 leading-relaxed max-w-2xl mx-auto mb-10 animate-fade-up animate-stagger-1" style={{ animationFillMode: 'both' }}>
          ...
          <div className="flex items-center justify-center gap-6 mt-6 animate-fade-up animate-stagger-3" style={{ animationFillMode: 'both' }}>
            {trustPoints.map((pt) => (
              <div key={pt} className="flex items-center gap-1.5">
                <CheckCircle size={13} className="text-teal-500 shrink-0" />
                <span className="font-sans text-xs text-slate-500">{pt}</span>
              </div>
            ))}
          </div>

          {/* Stats row */}
          <div className="mt-14 grid grid-cols-2 sm:grid-cols-4 gap-px bg-slate-200 rounded-2xl overflow-hidden border border-slate-200 shadow-card animate-fade-up animate-stagger-4" style={{ animationFillMode: 'both' }}>
            {stats.map(({ value, label }) => (
              <div key={label} className="bg-white px-6 py-5 text-center">
                <p className="font-mono text-2xl font-semibold text-gold-600">{value}</p>
      ...
      <section className="py-12 px-4 border-y border-slate-200 bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-start gap-6">
            <div className="shrink-0 w-12 h-12 rounded-xl bg-gold-50 border border-gold-200 flex items-center justify-center">
              <TrendingUp size={22} className="text-gold-600" />
            </div>
            <div>
              <p className="font-display text-xl font-semibold text-slate-800 mb-1 italic leading-snug">
              ...
              <p className="font-sans text-sm text-slate-500 mt-2">— 10 years of grant writing experience, distilled into every feature</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <p className="font-mono text-xs text-gold-600 uppercase tracking-widest mb-3">Expert Features</p>
            <h2 className="font-display text-4xl font-semibold text-slate-900 tracking-tight">
              Everything you need to win
            </h2>
            <p className="font-sans text-slate-600 mt-3 max-w-xl mx-auto leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map(({ icon: Icon, title, desc, accent }, i) => (
              <div
                key={title}
                className="card p-6 hover:border-slate-300 hover:shadow-card-md transition-all duration-200 hover:-translate-y-px group"
                style={{ animationDelay: `${i * 0.08}s` }}
              >
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4
                  ${accent === 'gold' ? 'bg-gold-50 border border-gold-200' : 'bg-teal-50 border border-teal-200'}`}>
                  <Icon size={18} className={accent === 'gold' ? 'text-gold-600' : 'text-teal-600'} />
                </div>
      ...
      {/* CTA — one intentional dark section for visual punch */}
      <section className="py-20 px-4 bg-navy-950">
        <div className="max-w-2xl mx-auto text-center">
          <p className="font-mono text-xs text-gold-500 uppercase tracking-widest mb-4">Get started today</p>
          <h2 className="font-display text-4xl font-semibold text-white tracking-tight mb-4">
      ...
      <footer className="border-t border-slate-200 bg-white py-8 px-4">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-display text-sm font-semibold text-slate-600">GrantFinder</span>
          </div>
