...
const STATUSES = [
  { value: 'draft',        label: 'Draft',        color: 'text-slate-600 bg-slate-100 border-slate-200' },
  { value: 'submitted',    label: 'Submitted',    color: 'text-amber-700 bg-amber-50 border-amber-200' },
  { value: 'under_review', label: 'Under Review', color: 'text-teal-700 bg-teal-50 border-teal-200' },
  { value: 'awarded',      label: 'Awarded ✓',    color: 'text-green-700 bg-green-50 border-green-200' },
  { value: 'rejected',     label: 'Rejected',     color: 'text-red-600 bg-red-50 border-red-200' },
]

interface AppForm {
  grant_title: string; status: string; amount_requested: string
  deadline: string; submitted_at: string; notes: string
}

const emptyForm: AppForm = {
  grant_title: '', status: 'draft', amount_requested: '',
  deadline: '', submitted_at: '', notes: '',
}
...
export default function Applications() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const payload = {
      user_id: user.id, grant_title: form.grant_title, status: form.status,
    ...
    <Layout>
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="font-display text-3xl font-semibold text-slate-900 tracking-tight">Applications</h1>
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-sans font-medium border transition-all duration-150
              ${filterStatus === '' ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'}`}
          >
            All ({apps.length})
          </button>
          {STATUSES.map(({ value, label }) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-sans font-medium border transition-all duration-150
                  ${filterStatus === value ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'}`}
              >
                {label} ({count})
              </button>
            )
        ...
        {showForm && (
          <div className="card p-6 mb-6 border-gold-200">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-sans font-semibold text-slate-900">{editId ? 'Edit Application' : 'Track New Application'}</h2>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <input type="text" value={form.grant_title} onChange={set('grant_title')}
                  className="input" placeholder="Grant title or funder name" required />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="number" value={form.amount_requested} onChange={set('amount_requested')}
                  className="input" placeholder="e.g. 50000" min="0" />
              </div>
              <div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filtered.map((app) => {
              ...
              return (
                <div key={app.id} className="card p-5 hover:border-slate-300 transition-all duration-150">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <div className="flex gap-1">
                        <select
                          ...
                          className="text-xs bg-white border border-slate-200 text-slate-700 rounded-lg px-2 py-1 focus:outline-none focus:border-navy-600"
                        >
                          {STATUSES.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
                        </select>
                        ...
                        <button
                          ...
                          className="btn-ghost text-xs px-2 py-1 text-slate-400 hover:text-red-500"
                        >
                          <X size={12} />
                        </button>
                      </div>
